...
const QuickCopyReferralButton: React.FC<QuickCopyReferralButtonProps> = ({
  ...
  const getTooltipClasses = () => {
    const base =
      'absolute z-50 px-2 py-1 text-xs bg-gray-900 text-white rounded whitespace-nowrap pointer-events-none transition-opacity';
    const positions = {
  ...
  if (variant === 'icon') {
    return (
      <div className="relative">
        ...
        >
          {copying ? <Check className="h-4 w-4 animate-pulse" /> : <Link className="h-4 w-4" />}
        </button>
        {showTooltip && (
          <div className={getTooltipClasses()}>{localize('com_ui_referral_copy_title')}</div>
        )}
      </div>
    );
  }
  ...
  if (variant === 'compact') {
    return (
      ...
      >
        {copying ? <Check className="h-4 w-4" /> : <Link className="h-4 w-4" />}
        {showLabel && (
  ...
  return (
    <button
      ...
      className={cn(
        ...
        'transition-all hover:bg-surface-tertiary',
        'dark:border-border-dark border border-border-light',
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="text-left">
          <p className="text-sm font-medium">
            {copying
              ? localize('com_ui_referral_copied_success')
              : localize('com_ui_referral_copy_link')}
          </p>
