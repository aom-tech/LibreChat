...
async function initializeGeminiVertexAI() {
  try {
...
async function saveImageLocally(base64Data, outputFormat = 'png', userId) {
  try {
 ...
 * @param {ServerRequest} req - Request object
 * @param {string} fileStrategy - File strategy
...
function validateConfig(override) {
  if (override) {
    return;
  }
 ...
 * @param {ServerRequest} fields.req - Request object
 ...
 * @param {MongoFile[]} [fields.imageFiles] - The images to be used for context
 * @param {Function} [fields.processFileURL] - Function to process file URLs
 * @param {string} [fields.userId] - User ID
 * @param {string} [fields.fileStrategy] - File storage strategy
 * @returns {Tool} - The Gemini image generation tool
 */
function createGeminiImageGenTool(fields = {}) {
  ...
  return tool(
    async ({ prompt, image_ids }, runnableConfig) => {
      if (!prompt) {
      ...
      logger.debug('[GeminiImageGen] Available imageFiles:', imageFiles.length);

      let ai;
      try {
        ai = await initializeGeminiVertexAI();
      } catch (error) {
        logger.error('[GeminiImageGen] Failed to initialize Gemini Vertex AI:', error);
        return returnValue(`Failed to initialize Gemini Vertex AI: ${error.message}`);
      }
      ...
      try {
        ...
        if (image_ids && image_ids.length > 0) {
          ...
          const contextImages = await convertImagesToInlineData(imageFiles, image_ids, req, fileStrategy);
        ...
        return returnValue(`Something went wrong when trying to generate the image. The Gemini Vertex AI API may be unavailable:
Error Message: ${error.message}`);
      ...
      if (fileStrategy === FileSources.local || fileStrategy === 'local') {
        ...
        try {
          await saveImageLocally(imageData, 'png', userId);
...
module.exports = createGeminiImageGenTool;
